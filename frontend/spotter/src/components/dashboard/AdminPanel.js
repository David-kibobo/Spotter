...
import { createDriver, fetchDrivers, fetchTrucks, addTruck, updateTruck, deleteTruck } from "../../api/endPoints";
...
const AdminPanel = () => {
  ...
  const handleTruckSubmit = (e) => {
    ...
    const formData = new FormData(e.target);
    const newTruck = {
      truck_number: formData.get("number"),  // Truck Number
    ...
    dispatch(addTruck(newTruck)); // Dispatch API call
    setShowTruckForm(false);
  };
      ...
      {showDriverForm && (
        <ModalOverlay>
          <Modal>
            <ModalHeader>
              <h3>{editMode ? "Edit Driver" : "Add Driver"}</h3>
              <CloseButton onClick={() => setShowDriverForm(false)}>×</CloseButton>
            </ModalHeader>
            <form onSubmit={handleDriverSubmit}>
              ...
              <Input type="text" name="truck" placeholder="Truck ID" />

              <ModalActions>
                <SaveButton type="submit">{editMode ? "Update" : "Save"}</SaveButton>
                <CancelButton onClick={() => setShowDriverForm(false)}>Cancel</CancelButton>
              </ModalActions>
            </form>
          </Modal>
        </ModalOverlay>
...
{showTruckForm && (
  <ModalOverlay>
    <Modal>
      <ModalHeader>
        <h3>{editMode ? "Edit Truck" : "Add Truck"}</h3>
        <CloseButton onClick={() => setShowTruckForm(false)}>×</CloseButton>
      </ModalHeader>
      <form onSubmit={handleTruckSubmit}>
        <Input type="text" name="number" placeholder="Truck Number" required />
        ...
        <ModalActions>
          <SaveButton type="submit">{editMode ? "Update" : "Save"}</SaveButton>
          <CancelButton onClick={() => setShowTruckForm(false)}>Cancel</CancelButton>
        </ModalActions>
      </form>
    </Modal>
  </ModalOverlay>
